...
from ppocr.infer.predict_system_v2 import TextSystem
...
def load_model(args, e2e_algorithm=False):
    ...
    # args.auto_rotate_whole_image = auto_rotate_whole_image
...
class OCR():
    def __init__(self, text_sys, img_str, cls = False, char_out = False, e2e_algorithm=False,auto_rotate_whole_image=False):
        ...
        if self.e2e_algorithm:
            ...
            self.rotated_img, self.dt_boxes, self.rec_res, self.rotated_angle = text_sys(img, cls, char_out, auto_rotate_whole_image)

    def __call__(self, union, max_x_dist = 50, min_y_overlap_ratio = 0.5,
                 rotated_img=None, dt_boxes=None, rec_res=None):
        if self.e2e_algorithm:
            img = self.img
        else:
...
if __name__ == "__main__":
    ...
    result = ocr(union = False, max_x_dist = 1000, min_y_overlap_ratio = 0.5)
